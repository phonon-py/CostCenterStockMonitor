...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.dir_output_btn.configure(width=10, height=1, command=self.login_google)
    ...
    def login_google(self):
        ...
        service = Service(executable_path=r"\\cpiweb\www\4930\ソフトウェア\MicrosoftEdge\WebDriver\122.0.2365.80\msedgedriver.exe")
        driver = webdriver.Edge(service=service, options=edge_options)

        """G.CIPにログインする"""
        driver.get('http://wggwap10.cgn.canon.co.jp/WBG/WBGE1/WBGED716.aspx')        

        sleep(1)                                                                       
        
        pyautogui.hotkey('win', 'up')

        # まず、WebDriverインスタンスを使用して<SELECT>タグを探します。
        select_element = driver.find_element(By.NAME, 'incfcompany')

        # 次に、Selectクラスを使用して<SELECT>タグをラップします。
        select = Select(select_element)

        # 最初の<OPTION>要素を選択します。
        select.select_by_index(10)

        driver.find_element(By.NAME, 'inuserid').send_keys(self.gcip_login_no_input.get())
        driver.find_element(By.NAME, 'inpasswd').send_keys(self.gcip_pass_input.get())
        driver.find_element(By.NAME, "login").click()
        sleep(0.5) # 0.1秒待機

        select_element = driver.find_element(By.NAME, 'ddl_QT_INVENT')
        select = Select(select_element)
        select.select_by_index(3)
        sleep(0.1) # 0.1秒待機
        driver.find_element(By.NAME, 'txt_CD_BLOCK').send_keys('3790')
        sleep(0.1) # 0.1秒待機
        ...
        sleep(1) # 1秒待機      
        pyperclip.copy(self.dir_save_path.cget("text"))
        pyautogui.hotkey('ctrl', 'v')
        sleep(1) 
        pyautogui.press('enter')
        sleep(1) 
